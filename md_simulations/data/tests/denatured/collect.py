...
sequences = pd.read_csv('den_seq.csv',index_col=0)
...
for name in sequences.index:
    ...
    for replica in range(5):
        ...
        Rg.append(analysis.loc['Rg','value'])
    ...
    sequences.loc[name,'Rg'] = np.mean(Rg)
    sequences.loc[name,'Rg_err'] = np.std(Rg)
...
sequences.to_csv('conf_den_seq.csv')
