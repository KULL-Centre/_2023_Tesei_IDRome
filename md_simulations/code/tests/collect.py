...
sequences = pd.read_csv('rg_test_sequences.csv',index_col=0)
for name in sequences.index:
    ...
    for replica in range(5):
        ...
        ree = np.load(name+f'/{replica:d}/ete.npy')
        Rg_mean.append(rg.mean())
        rel_rg_err.append( (rg.mean()-sequences.loc[name].exp_rg)/sequences.loc[name].exp_rg*100 )
        Ree_mean.append(ree.mean())
        rg_array.append(rg)
        ree_array.append(ree)
    ...
    sequences.loc[name,'Rg'] = np.mean(Rg_mean)
    sequences.loc[name,'Rg_err'] = np.std(Rg_mean)
    sequences.loc[name,'rel_rg_err'] = np.mean(rel_rg_err)
    sequences.loc[name,'rel_rg_err_std'] = np.std(rel_rg_err)
    sequences.loc[name,'rel_rg_err_replicas'] = ', '.join([str(x) for x in rel_rg_err])
    ...
    np.save(name+'_rg_array.npy',np.array(rg_array).flatten())
    np.save(name+'_ree_array.npy',np.array(ree_array).flatten())
    np.save(name+'_Delta_array.npy',np.array(Delta_array).flatten())
    np.save(name+'_S_array.npy',np.array(S_array).flatten())
    print(name)
    time.sleep(.6)

sequences.to_csv('rg_test_data.csv')
