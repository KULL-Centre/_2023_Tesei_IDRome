...
sequences = pd.read_csv('PED_sequences.csv',index_col=0,dtype=object)
for name in sequences.index:
    ...
    pd.DataFrame(index=range(nres),columns=range(nres),data=np.mean(cmap,axis=0)).to_csv(name+'_cmap.csv.gz')
